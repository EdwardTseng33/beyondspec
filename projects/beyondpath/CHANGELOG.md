...
## v1.12.1
<!-- v1.12.1 · AI Native 體質檢測→改名「AI能力檢測」(模組總名+個人/團隊雙入口+訪談官/評分官/報告全改) · 修「側邊欄點不開」bug(Router routes 補登記 health→點擊正確開啟健檢頁) · 設定頁「模組管理·你的工作台」補上 AI能力檢測 開關(MODULE_DEFAULTS/MIN_PLAN 登記 mod-health=free·免費可用) · 歸類=個人驗證工具(與產品力診斷同欄) · sidebar v1.12.0→v1.12.1 -->

## v1.12.2
<!-- v1.12.2 · AI能力檢測模組整體設計重整（女巫 Gate2 主導·蘇菲審）：①根治版面靠左空蕩(.hc-wrap 補 margin:0 auto 置中) ②入口卡 滿版色磚→白底卡+色票塊(封閉五色語意) ③進入頁 編號說明卡+儀式分隔線+層次 ④訪談頁 修 var(--bg) 落空致捲動穿透(→var(--bg-app)) + AI 訊息加氣泡容器(原裸文字) + 進度條加粗 ⑤報告頁 寫死琥珀→複用 --amber-* token + 暖底修正(原灰底) + KPI hover ⑥全暗色變數化·對比達標·去漸層飽和純色 ⑦鍵盤可達+隱私文案校正 · rubric 86/100 · sidebar v1.12.1→v1.12.2 -->

## v1.12.3
<!-- v1.12.3 · AI能力檢測對照 Claude Design 定稿設計（Edward 親修版）第 1 批視覺對齊：①六維改名全鏈(工作流/工具/判斷/領域/協作/交付 → 提問框架/批判視野/任務拆解/工具整合/反思迭代/領域提案；DIMS + 評分官 prompt + 訪談 self 段同步、id 不動所以評分邏輯安全) ②報告加「由 AI Native Coach OS 評定」+ 雷達標 Coach OS 模型 ③等級色階對齊(A+ 金→紫深 #5B3FC9、S 紫深→金 #E08A3A 對調) ④雙入口文案對齊(個人卡描述 + 按鈕統一「去檢測」) · 六邊形徽章樣式/訪談數據條/作品提交階段列第 2 批 · sidebar v1.12.2→v1.12.3 -->

## v1.12.4
<!-- v1.12.4 · AI能力檢測第 2 批(部分)·六邊形徽章：報告主徽章 + 能力階梯 從線條 icon 改「六邊形 + 等級字母」(對齊 Claude Design 定稿)；新增 hexBadge helper(正六邊形 SVG + 白字垂直置中)；色階 B灰#94A3B8 / B+綠#1BA891 / A紫#7C5CFC / A+紫深#5B3FC9 / S金#E08A3A · 訪談右側數據條 + 作品提交階段續做 · sidebar v1.12.3→v1.12.4 -->

## v1.12.5
<!-- v1.12.5 · AI能力檢測第 2 批·作品提交 + AI 審核 + 認證（Edward 選 B 完整功能版·卡西法實作·蘇菲審）：①訪談後新增「上傳作品」階段(作品連結 + Git Repo + 作品說明) ②AI 審核作品(callV2·依作品說明與連結評定·誠實標示 AI 不存取連結實際內容·有 fallback) ③報告新增「AI Native Coach OS 認證」區(六邊形認證徽章 + 等級 B/A/S + 評語 + 亮點 + 作品連結) ④跳過→顯示未提交引導 + 補交按鈕 ⑤work/cert 隨報告存檔 · 暗色評語對比優化 · Gate1 完整流程實測(訪談→作品→AI審核→認證報告 render 全對 + 零 console error) · sidebar v1.12.4→v1.12.5 -->

## v1.12.6
<!-- v1.12.6 · AI能力檢測接上點數扣除（Edward 要求：考核用到的 AI 也要消耗點數）：①開始檢測扣 10 點(涵蓋訪談 6 段 AI 追問 + 評分) ②送作品審核扣 5 點(AI 審核作品) ③餘額不足擋下並提示升級 ④useCredits/checkCredits 暴露 window(原 app scope 健檢模組呼叫不到、扣點失效·已修) · Gate1 實測扣點生效(開始檢測餘額 -10 確認·零 error) · 點數值 10/5 可調 · sidebar v1.12.5→v1.12.6 -->
...
## v1.12.9
<!-- v1.12.9 · AI能力檢測 UIUX 修復（Edward 暴怒「爆按鈕」後·全程真 Chrome 實測每頁）：①所有按鈕箭頭補尺寸——送AI審核/訪談送出/匯出PDF/分享/重新檢測/補交作品/把報告變行動 原 SVG 無 width/height→撐爆成巨大方塊·健檢段 24x24 SVG 統一補 16x16（有 CSS 尺寸者不受影響） ②對話氣泡 Georgia 襯線 16px→Noto Sans 黑體 14px(對齊設計 FRAME3·圓角 4/14/14/14)·用戶氣泡同步 14px ③訪談右欄 checklist 名稱對齊訪談主題(認識你/AI工具現況/倫理紅線/拿手工作流/判斷與查證/自我評估)·原誤套六維名又漏反思迭代→混亂 ④訪談加最低字數 gate(<6字)+shake 動畫+toast——隨便答「好」被擋下、值保留可補(解 Edward「隨便回答就進下一步」)·正常答案不誤擋 · 全程真 Chrome 截圖實測 作品/訪談/報告 三頁(Preview 3.3MB 凍結改用真 browser)·JS 語法已驗(_HC 正常載入)·sidebar v1.12.8→v1.12.9 -->
...
## v1.12.15
<!-- v1.12.15 · AI能力檢測 沒作品封頂 B+（Edward「我沒提供作品評分也可以 A?? 莫名其妙」）：①根因=等級只看訪談六維分數算(calcLScore+calcTier)、作品(work)只存進結果不算等級→沒交作品光訪談說得好就給 A ②buildResult 加封頂：沒作品(work=null)且訪談 rawTier > B+ → 等級封頂 B+（A/A+/S 必須作品佐證·對齊 BeyondPath 證據導向「沒證據不給高分」）·結果存 rawTierIdx + capped 旗標 ③報告 hero 加封頂提示卡(暖色 gold·非漸層)：「你在訪談中展現了 X 級的實力——但 A 以上認證需作品佐證·交一份作品解鎖·目前認證等級 B+」· 真 Chrome 實測：A+ 實力訪談 沒作品→封頂 B+(capped)、有作品→A+(不封頂)·提示卡視覺確認 · ⚠️報告版面對齊設計檔雙欄(設計檔左等級+階梯/右六維拆解·現為單欄+多三數字卡) 下輪照設計檔重排 · sidebar v1.12.14→v1.12.15 -->
